...
The interactive UI render action may request a local video render for the selected launch pack. The render route should prefer a server-side launch-pack lookup and may accept a full `launchPack` fallback when local serverless storage cannot find the id.
