...
4. `lib/demo-video-service.ts` creates a `DemoVideoProject` with pending proof review and a pending HyperFrames render spec.
5. `POST /api/demo-videos/:id/proof-review` rewrites captions and voiceover script from accepted claims.
6. `POST /api/demo-videos/:id/render` calls `lib/demo-video-renderer.ts`.
7. The renderer prepares screenshots, requests Gradium WAV audio when configured, renders HyperFrames MP4, and optionally muxes audio through FFmpeg.
8. Supabase storage receives `demo-video.mp4` and `voiceover.wav` when admin credentials are present.
...
- `ProofReview`: accepted and rejected claim ids.
...
- Tavily: optional research context for proof-aware claims.
